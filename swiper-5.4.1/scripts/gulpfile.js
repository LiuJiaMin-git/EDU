...
gulp.task('playground', (cb) => {
  ...
  let content = fs.readFileSync(path.resolve(__dirname, '../playground/2_栅栏布局.html'), 'utf8');
  if (env === 'development') {
    content = content
  ...
  fs.writeFileSync(path.resolve(__dirname, '../playground/2_栅栏布局.html'), content);
  if (cb) cb();
});
gulp.task('js', (cb) => {
  buildJs(cb);
...
gulp.task('open', () => {
  gulp.src('./playground/2_栅栏布局.html').pipe(gopen({ uri: 'http://localhost:3000/playground/' }));
});
